...
# Buscar todas las carpetas que contengan rosbag dentro de folder_prefix
rosbag_dirs = [d for d in os.listdir(folder_prefix)
               if os.path.isdir(os.path.join(folder_prefix, d)) and d.startswith("rosbag_")]
# --- Crear base SQLite de salida ---
db_path = folder_prefix + "merged_database.db"
conn = sqlite3.connect(db_path)
"""# Carpeta para imagenes
image_folder = folder_prefix + "database_img"
...
for rosbag_file in rosbag_dirs:
  bag_path = os.path.join(folder_prefix, rosbag_file)
  ...
  # --- Leer el mensaje con info de acierto o fallo
  ...
  count = 1
  img_count = 0
  print(f"=== Procesando {rosbag_file} ===")
  print("task: {}, try: {}, success: {}".format(task, attempt, success))
