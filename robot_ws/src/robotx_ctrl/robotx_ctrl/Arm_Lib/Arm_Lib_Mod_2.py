...
class Arm_Device(object):
    ...
    def Arm_serial_servo_write6_array(self, joints:List[float], time:int)->None:
        ...
        if s1 > pi or s2 > pi or s3 > pi or s4 > pi or s5 > pi*(3/4.0) or s6 > pi:
            ...
            return
        try:
            pos = int((3100 - 900) * (s1 - 0) / (pi - 0) + 900)
            value1_H = (pos >> 8) & 0xFF
            value1_L = pos & 0xFF

            s2 = pi - s2
            pos = int((3100 - 900) * (s2 - 0) / (pi - 0) + 900)
            value2_H = (pos >> 8) & 0xFF
            value2_L = pos & 0xFF

            s3 = pi - s3
            pos = int((3100 - 900) * (s3 - 0) / (pi - 0) + 900)
            value3_H = (pos >> 8) & 0xFF
            value3_L = pos & 0xFF

            s4 = pi - s4
            pos = int((3100 - 900) * (s4 - 0) / (pi - 0) + 900)
            value4_H = (pos >> 8) & 0xFF
            value4_L = pos & 0xFF

            pos = int((3700 - 380) * (s5 - 0) / ((pi*(3/4.0)) - 0) + 380)
            value5_H = (pos >> 8) & 0xFF
            value5_L = pos & 0xFF

            pos = int((3100 - 900) * (s6 - 0) / (pi - 0) + 900)
            value6_H = (pos >> 8) & 0xFF
            value6_L = pos & 0xFF
            time_H = (time >> 8) & 0xFF
            time_L = time & 0xFF
    ...
    def Arm_serial_servo_read(self, id:int)->float:
        ...
        if id == 5:
            #pos = (float(pi*(3/4.0)) - 0) * (pos - 380) / (3700 - 380) + 0
            pos = ((float(pi*(3/2.0)) - 0) * (pos - 380) / (3700 - 380)) - float(pi/2.0) #0-180
            """if pos > pi*(3/4.0):
                return float(pi*(3/4.0))
            if pos < 0:
                return float(0)"""
            #if pos > pi*(3/4.0) or pos < 0:
            #    return None
        else:
            pos = ((float(pi) - 0) * (pos - 900) / (3100 - 900)) - float(pi/2.0)
